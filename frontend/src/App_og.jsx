...
function ChatPanel({ result, labText, analysisType }) {
  ...
  return (
    <div style={{
      ...
      {/* Header */}
      <div style={{ background: "linear-gradient(135deg, #1dbaa5, #3cc7dc)", borderRadius: "12px 12px 0 0", padding: "16px 20px", color: "white" }}>
...
function LifestyleForm({ onSubmit, extracting, analyzing }) {
  ...
  const familyOptions = ["Diabetes", "Heart Disease", "Cancer", "Kidney Disease", "Hypertension", "None"];
      ...
      <div style={{ background: "white", borderRadius: 12, padding: 32, boxShadow: "0 2px 12px rgba(0,0,0,0.08)" }}>
        ...
        <div
          ...
          style={{
            ...
            borderRadius: 10, padding: "36px 20px", textAlign: "center",
            cursor: "pointer", background: file ? "#f0f7ff" : "#fafafa", transition: "all 0.2s"
          }}
        >
          <div style={{ fontSize: 40 }}>📄</div>
          <p style={{ margin: "10px 0 4px", fontWeight: 600, color: "#333" }}>
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f5f7fa", fontFamily: "'Segoe UI', sans-serif" }}>
      ...
      <div style={{ background: "linear-gradient(135deg, #ba981d, #3cdca4)", padding: "24px 40px", color: "white" }}>
        <h1 style={{ margin: 0, fontSize: 26, fontWeight: 700 }}>🧬 Medical AI Analyzer</h1>
      ...
      {/* Tabs */}
      <div style={{ background: "white", borderBottom: "1px solid #e8e8e8", padding: "0 40px", display: "flex", gap: 4 }}>
        {TABS.map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{
            ...
            color: activeTab === tab.id ? "#dc6df6" : "#666",
